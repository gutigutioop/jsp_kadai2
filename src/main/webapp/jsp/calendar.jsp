...
<%!
    private static Map eventMap = new HashMap(); //mapに対して値を保持。　static:jspの中でメモリー上にmap保持する。
  static {
...
<%
  ...
  // 画面で利用するための日付、イベント情報を保存  //jsp暗黙オブジェクト
  request.setAttribute("dates", dates);  
  request.setAttribute("date", Date.from(localDate.atStartOfDay(ZoneId.systemDefault()).toInstant()));
  //ZoneId じじょうほう。今の自分がどこにいるか。systemDefault　コンピュータに指定された場所に移動。toInstant　ある一定の場所のその瞬間。
  //文字列に変換している。
